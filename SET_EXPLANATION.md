# Множества (Set) в Redis - Подробное объяснение

## Что такое множество в Redis?

**Множество (Set)** - это неупорядоченная коллекция уникальных строк в Redis. Каждый элемент может встречаться только один раз.

Представьте множество как Set в JavaScript или множество в математике:
- Все элементы уникальны (дубликаты автоматически удаляются)
- Порядок элементов не гарантирован
- Быстрый поиск элемента (O(1))
- Поддержка операций над множествами (объединение, пересечение, разность)

## Ключевые особенности множеств

1. **Уникальность** - каждый элемент может быть только один раз
2. **Неупорядоченность** - порядок элементов не гарантирован
3. **Быстрый поиск** - проверка наличия элемента за O(1)
4. **Операции над множествами** - объединение, пересечение, разность

## Визуальное представление

```
Множество: tags
┌─────────┐
│  "red"  │
│  "blue" │
│ "green" │
│ "black" │
└─────────┘
(порядок не важен, дубликаты невозможны)
```

## Аналогия: Мешок с уникальными предметами

Представьте множество как мешок с уникальными предметами:
- Каждый предмет может быть только один раз
- Порядок не важен
- Можно быстро проверить, есть ли предмет в мешке
- Можно объединять мешки, находить общие предметы и т.д.

## Методы работы с множествами

### 1. SADD - Добавить элементы в множество

```typescript
await redis.sadd('tags', 'red', 'blue', 'green');
```

**Что происходит:**
- Элементы добавляются в множество
- Если элемент уже существует, он игнорируется (не добавляется повторно)
- Если множество не существует, оно создается
- Можно добавить несколько элементов за раз
- Возвращает количество новых элементов (не существовавших ранее)

**Пример:**
```typescript
// Добавляем элементы
const added1 = await redis.sadd('tags', 'red', 'blue');
console.log(added1); // 2 (оба элемента новые)

// Пытаемся добавить снова (red уже есть)
const added2 = await redis.sadd('tags', 'red', 'green');
console.log(added2); // 1 (только green был новым, red проигнорирован)

// Множество теперь: {'red', 'blue', 'green'}
```

**Визуально:**
```
До:     {}
        ↓ SADD('red', 'blue')
После:  {'red', 'blue'}

        ↓ SADD('red', 'green')  (red уже есть)
После:  {'red', 'blue', 'green'}  (red проигнорирован)
```

### 2. SMEMBERS - Получить все элементы множества

```typescript
const tags = await redis.smembers('tags');
// Вернет: ['red', 'blue', 'green'] (порядок может быть любым)
```

**Что происходит:**
- Возвращаются все элементы множества
- Порядок элементов не гарантирован
- Если множество не существует, возвращается пустой массив

**Пример:**
```typescript
await redis.sadd('tags', 'red', 'blue', 'green');
const allTags = await redis.smembers('tags');
console.log(allTags); // ['blue', 'green', 'red'] (порядок может отличаться)
```

**Важно:** Порядок элементов может быть разным при каждом вызове!

### 3. SREM - Удалить элементы из множества

```typescript
const removed = await redis.srem('tags', 'red', 'blue');
// Удалит 'red' и 'blue' из множества
```

**Что происходит:**
- Удаляет указанные элементы из множества
- Если элемент не существует, он игнорируется
- Можно удалить несколько элементов за раз
- Возвращает количество удаленных элементов

**Пример:**
```typescript
// Множество: {'red', 'blue', 'green'}
const removed1 = await redis.srem('tags', 'red', 'yellow');
console.log(removed1); // 1 (удален только red, yellow не существовал)
// Множество после: {'blue', 'green'}
```

**Визуально:**
```
До:     {'red', 'blue', 'green'}
        ↓ SREM('red', 'yellow')
После:  {'blue', 'green'}
        ↑
    Удален: red (yellow не существовал)
```

### 4. SISMEMBER - Проверить наличие элемента

```typescript
const exists = await redis.sismember('tags', 'red');
// Вернет: 1 (если есть) или 0 (если нет)
```

**Что происходит:**
- Проверяет, является ли элемент членом множества
- Возвращает 1, если элемент существует, 0 - если нет
- Очень быстрая операция (O(1))

**Пример:**
```typescript
await redis.sadd('tags', 'red', 'blue');

const hasRed = await redis.sismember('tags', 'red');
console.log(hasRed); // 1 (есть)

const hasYellow = await redis.sismember('tags', 'yellow');
console.log(hasYellow); // 0 (нет)
```

**Визуально:**
```
Множество: {'red', 'blue', 'green'}

SISMEMBER('red')    → 1 (есть)
SISMEMBER('yellow') → 0 (нет)
```

## Практические примеры использования

### Пример 1: Теги для статей

```typescript
// Добавляем теги к статье
await redis.sadd('article:1:tags', 'javascript', 'redis', 'tutorial');
await redis.sadd('article:2:tags', 'javascript', 'nodejs', 'tutorial');

// Получаем все теги статьи
const article1Tags = await redis.smembers('article:1:tags');
console.log(article1Tags); // ['javascript', 'redis', 'tutorial']

// Проверяем, есть ли тег
const hasRedis = await redis.sismember('article:1:tags', 'redis');
if (hasRedis) {
  console.log('Статья содержит тег redis');
}

// Удаляем тег
await redis.srem('article:1:tags', 'tutorial');
```

### Пример 2: Уникальные посетители

```typescript
// Отслеживание уникальных посетителей страницы
async function trackVisitor(pageId: string, userId: string) {
  // Добавляем пользователя в множество посетителей
  const isNew = await redis.sadd(`page:${pageId}:visitors`, userId);
  
  if (isNew === 1) {
    console.log(`Новый посетитель: ${userId}`);
  } else {
    console.log(`Повторный посетитель: ${userId}`);
  }
}

// Получаем всех уникальных посетителей
async function getUniqueVisitors(pageId: string) {
  return await redis.smembers(`page:${pageId}:visitors`);
}

// Использование
await trackVisitor('home', 'user123');
await trackVisitor('home', 'user456');
await trackVisitor('home', 'user123'); // Повторный визит

const visitors = await getUniqueVisitors('home');
console.log(visitors); // ['user123', 'user456'] (только уникальные)
```

### Пример 3: Черный список пользователей

```typescript
class BlacklistService {
  constructor(private redis: RedisService) {}

  // Добавить пользователя в черный список
  async banUser(userId: string) {
    await this.redis.sadd('blacklist', userId);
  }

  // Проверить, забанен ли пользователь
  async isBanned(userId: string): Promise<boolean> {
    const result = await redis.sismember('blacklist', userId);
    return result === 1;
  }

  // Удалить пользователя из черного списка
  async unbanUser(userId: string) {
    await this.redis.srem('blacklist', userId);
  }

  // Получить всех забаненных пользователей
  async getAllBanned(): Promise<string[]> {
    return await this.redis.smembers('blacklist');
  }
}
```

### Пример 4: Избранное пользователя

```typescript
// Добавляем товар в избранное
await redis.sadd('user:1:favorites', 'product:123');
await redis.sadd('user:1:favorites', 'product:456');

// Проверяем, есть ли товар в избранном
const isFavorite = await redis.sismember('user:1:favorites', 'product:123');
if (isFavorite) {
  console.log('Товар в избранном');
}

// Получаем все избранные товары
const favorites = await redis.smembers('user:1:favorites');
console.log(favorites); // ['product:123', 'product:456']

// Удаляем из избранного
await redis.srem('user:1:favorites', 'product:123');
```

### Пример 5: Уникальные IP адреса

```typescript
// Логирование уникальных IP адресов
async function logIP(ip: string) {
  const isNew = await redis.sadd('unique:ips', ip);
  if (isNew === 1) {
    console.log(`Новый уникальный IP: ${ip}`);
  }
}

// Получаем все уникальные IP
async function getUniqueIPs(): Promise<string[]> {
  return await redis.smembers('unique:ips');
}

// Использование
await logIP('192.168.1.1');
await logIP('192.168.1.2');
await logIP('192.168.1.1'); // Повторный IP (не добавится)

const uniqueIPs = await getUniqueIPs();
console.log(uniqueIPs); // ['192.168.1.1', '192.168.1.2']
```

## Дополнительные операции над множествами

### 5. SCARD - Получить количество элементов в множестве

```typescript
const count = await redis.scard('tags');
// Вернет количество элементов в множестве
```

**Что происходит:**
- Возвращает количество элементов в множестве
- Если множество не существует, возвращает 0
- Аналог `LLEN` для списков

**Пример:**
```typescript
await redis.sadd('tags', 'red', 'blue', 'green');
const count = await redis.scard('tags');
console.log(count); // 3
```

### 6. SPOP - Получить и удалить случайный элемент

```typescript
const random = await redis.spop('tags');
// Вернет случайный элемент и удалит его из множества
```

**Что происходит:**
- Возвращает случайный элемент из множества
- Элемент удаляется из множества
- Если множество пусто, возвращает `null`

**Пример:**
```typescript
// Множество: {'red', 'blue', 'green'}
const random1 = await redis.spop('tags');
console.log(random1); // Например: 'blue'
// Множество после: {'red', 'green'}

const random2 = await redis.spop('tags');
console.log(random2); // Например: 'green'
// Множество после: {'red'}
```

**Использование:** Лотереи, случайные выборки, очереди с случайным порядком

### 7. SRANDMEMBER - Получить случайный элемент (без удаления)

```typescript
const random = await redis.srandmember('tags');
// Вернет случайный элемент, но не удалит его
```

**Параметры:**
- `key` - имя ключа множества
- `count` - опционально: количество элементов (если не указано, возвращает один элемент)

**Что происходит:**
- Возвращает случайный элемент из множества
- Элемент остается в множестве (не удаляется)
- Если указан `count`, возвращает массив элементов
- Если множество пусто, возвращает `null`

**Примеры:**
```typescript
// Множество: {'red', 'blue', 'green'}

// Получить один случайный элемент
const random = await redis.srandmember('tags');
console.log(random); // Например: 'blue'
// Множество остается: {'red', 'blue', 'green'}

// Получить несколько случайных элементов
const randomMultiple = await redis.srandmember('tags', 2);
console.log(randomMultiple); // Например: ['red', 'green']
```

**Использование:** Показ случайных элементов без удаления, предпросмотр

### 8. SMOVE - Переместить элемент из одного множества в другое

```typescript
const moved = await redis.smove('set1', 'set2', 'element');
// Переместит 'element' из set1 в set2
```

**Что происходит:**
- Атомарно перемещает элемент из одного множества в другое
- Если элемент не существует в исходном множестве, ничего не происходит
- Если целевое множество не существует, оно создается
- Возвращает 1, если элемент перемещен, 0 - если не найден

**Пример:**
```typescript
// set1: {'a', 'b', 'c'}
// set2: {'d', 'e'}

const moved = await redis.smove('set1', 'set2', 'b');
console.log(moved); // 1 (элемент перемещен)

// set1 после: {'a', 'c'}
// set2 после: {'d', 'e', 'b'}
```

**Использование:** Перемещение элементов между категориями, изменение статусов

### 9. SUNION - Объединить несколько множеств

```typescript
const union = await redis.sunion('set1', 'set2', 'set3');
// Вернет все уникальные элементы из всех множеств
```

**Что происходит:**
- Объединяет несколько множеств
- Возвращает все уникальные элементы из всех множеств
- Дубликаты автоматически удаляются
- Исходные множества не изменяются

**Пример:**
```typescript
// set1: {'a', 'b', 'c'}
// set2: {'c', 'd', 'e'}
// set3: {'e', 'f'}

const union = await redis.sunion('set1', 'set2', 'set3');
console.log(union); // ['a', 'b', 'c', 'd', 'e', 'f']
// Исходные множества не изменились
```

**Визуально:**
```
set1: {'a', 'b', 'c'}
set2: {'c', 'd', 'e'}
set3: {'e', 'f'}

SUNION → {'a', 'b', 'c', 'd', 'e', 'f'}
```

**Использование:** Объединение тегов, поиск по нескольким категориям

### 10. SINTER - Найти пересечение множеств (общие элементы)

```typescript
const intersection = await redis.sinter('set1', 'set2', 'set3');
// Вернет элементы, которые есть во всех множествах
```

**Что происходит:**
- Находит элементы, которые присутствуют во всех указанных множествах
- Возвращает только общие элементы
- Исходные множества не изменяются

**Пример:**
```typescript
// set1: {'a', 'b', 'c', 'd'}
// set2: {'b', 'c', 'd', 'e'}
// set3: {'c', 'd', 'f'}

const intersection = await redis.sinter('set1', 'set2', 'set3');
console.log(intersection); // ['c', 'd'] (только общие элементы)
```

**Визуально:**
```
set1: {'a', 'b', 'c', 'd'}
set2: {'b', 'c', 'd', 'e'}
set3: {'c', 'd', 'f'}

SINTER → {'c', 'd'} (только то, что есть везде)
```

**Использование:** Поиск общих интересов, фильтрация по нескольким критериям

### 11. SDIFF - Найти разность множеств

```typescript
const diff = await redis.sdiff('set1', 'set2', 'set3');
// Вернет элементы из set1, которых нет в set2 и set3
```

**Что происходит:**
- Возвращает элементы из первого множества, которых нет в остальных
- Вычитает из первого множества все остальные
- Исходные множества не изменяются

**Пример:**
```typescript
// set1: {'a', 'b', 'c', 'd'}
// set2: {'b', 'c'}
// set3: {'d'}

const diff = await redis.sdiff('set1', 'set2', 'set3');
console.log(diff); // ['a'] (только то, что есть в set1, но нет в set2 и set3)
```

**Визуально:**
```
set1: {'a', 'b', 'c', 'd'}
set2: {'b', 'c'}
set3: {'d'}

SDIFF → {'a'} (set1 минус set2 минус set3)
```

**Использование:** Найти уникальные элементы, исключить элементы из другого множества

## Сравнение с другими структурами данных

### Set vs List (Список)

| Характеристика | Set (Множество) | List (Список) |
|----------------|-----------------|---------------|
| Уникальность | ✅ Да (дубликаты невозможны) | ❌ Нет (дубликаты разрешены) |
| Порядок | ❌ Не гарантирован | ✅ Сохраняется |
| Поиск элемента | ✅ O(1) - очень быстро | ❌ O(N) - медленно |
| Добавление | ✅ O(1) | ✅ O(1) |
| Получение всех | ✅ SMEMBERS | ✅ LRANGE |
| Использование | Теги, уникальные ID, черные списки | Очереди, стеки, история |

### Set vs Hash (Хеш)

| Характеристика | Set (Множество) | Hash (Хеш) |
|----------------|-----------------|------------|
| Структура | Коллекция значений | Поля-значения |
| Поиск | ✅ O(1) - по значению | ✅ O(1) - по полю |
| Уникальность | ✅ Значения уникальны | ✅ Поля уникальны |
| Использование | Теги, уникальные коллекции | Объекты, профили |

## Паттерны использования

### 1. Теги и категории

```typescript
// Добавляем теги к контенту
await redis.sadd('article:1:tags', 'tech', 'programming', 'redis');

// Находим статьи с определенным тегом (требует дополнительной логики)
// Обычно используется обратный индекс:
await redis.sadd('tag:tech:articles', 'article:1', 'article:2');
await redis.sadd('tag:programming:articles', 'article:1', 'article:3');
```

### 2. Уникальные идентификаторы

```typescript
// Отслеживание уникальных пользователей
await redis.sadd('unique:users', 'user:1', 'user:2', 'user:1');
// Результат: {'user:1', 'user:2'} (дубликат автоматически удален)
```

### 3. Фильтрация и проверка принадлежности

```typescript
// Черный список
await redis.sadd('blacklist', 'user:123', 'user:456');

// Проверка перед действием
const isBanned = await redis.sismember('blacklist', userId);
if (isBanned) {
  throw new Error('User is banned');
}
```

### 4. Счетчики уникальных событий

```typescript
// Уникальные просмотры страницы
await redis.sadd('page:views:unique', userId);
const uniqueViews = await redis.smembers('page:views:unique');
const count = uniqueViews.length;
```

## Важные особенности

### ✅ Преимущества множеств:
- **Уникальность** - автоматическое удаление дубликатов
- **Быстрый поиск** - проверка наличия за O(1)
- **Эффективность памяти** - хранит только уникальные значения
- **Операции над множествами** - объединение, пересечение, разность

### ⚠️ Ограничения:
- Порядок элементов не гарантирован
- Нельзя получить элемент по индексу
- Все элементы должны быть строками

### 💡 Когда использовать множества:
- Теги и категории
- Уникальные идентификаторы
- Черные/белые списки
- Избранное
- Уникальные посетители/события
- Любые случаи, где важна уникальность

### ❌ Когда НЕ использовать множества:
- Нужен порядок элементов (используйте List)
- Нужна сортировка (используйте Sorted Set)
- Нужны связанные данные (используйте Hash)
- Нужны дубликаты (используйте List)

## Практические примеры использования дополнительных операций

### Пример 1: Поиск статей с несколькими тегами

```typescript
// Статьи с разными тегами
await redis.sadd('tag:tech:articles', 'article:1', 'article:2', 'article:3');
await redis.sadd('tag:programming:articles', 'article:2', 'article:3', 'article:4');
await redis.sadd('tag:redis:articles', 'article:1', 'article:3', 'article:5');

// Найти статьи, которые имеют ВСЕ три тега (пересечение)
const articlesWithAllTags = await redis.sinter(
  'tag:tech:articles',
  'tag:programming:articles',
  'tag:redis:articles'
);
console.log(articlesWithAllTags); // ['article:3'] - только эта статья имеет все три тега

// Найти статьи с любым из тегов (объединение)
const articlesWithAnyTag = await redis.sunion(
  'tag:tech:articles',
  'tag:programming:articles',
  'tag:redis:articles'
);
console.log(articlesWithAnyTag); // Все уникальные статьи
```

### Пример 2: Лотерея с призами

```typescript
// Создаем множество призов
await redis.sadd('prizes', 'car', 'bike', 'phone', 'watch', 'book');

// Разыгрываем призы (удаляем из множества)
const prize1 = await redis.spop('prizes'); // Например: 'car'
const prize2 = await redis.spop('prizes'); // Например: 'bike'

// Проверяем оставшиеся призы
const remaining = await redis.scard('prizes');
console.log(`Осталось призов: ${remaining}`); // 3

// Показываем случайный приз без удаления (для предпросмотра)
const preview = await redis.srandmember('prizes');
console.log(`Следующий приз может быть: ${preview}`);
```

### Пример 3: Управление подписками пользователей

```typescript
// Пользователь подписан на категории
await redis.sadd('user:1:subscriptions', 'tech', 'sports', 'music');
await redis.sadd('user:2:subscriptions', 'tech', 'news', 'music');
await redis.sadd('user:3:subscriptions', 'sports', 'news');

// Найти общие интересы пользователей (пересечение)
const commonInterests = await redis.sinter(
  'user:1:subscriptions',
  'user:2:subscriptions'
);
console.log(commonInterests); // ['tech', 'music']

// Найти уникальные подписки пользователя 1 (разность)
const uniqueToUser1 = await redis.sdiff(
  'user:1:subscriptions',
  'user:2:subscriptions'
);
console.log(uniqueToUser1); // ['sports'] - только у пользователя 1

// Переместить подписку из активных в архив
await redis.smove('user:1:subscriptions', 'user:1:archived', 'sports');
```

### Пример 4: Система рекомендаций

```typescript
// Теги, которые нравятся пользователю
await redis.sadd('user:1:likes', 'action', 'comedy', 'drama');
await redis.sadd('user:2:likes', 'comedy', 'drama', 'romance');
await redis.sadd('user:3:likes', 'action', 'horror', 'thriller');

// Найти пользователей с похожими вкусами
const similarToUser1 = await redis.sinter('user:1:likes', 'user:2:likes');
console.log(similarToUser1); // ['comedy', 'drama'] - общие предпочтения

// Найти уникальные предпочтения для рекомендаций
const uniqueLikes = await redis.sdiff('user:2:likes', 'user:1:likes');
console.log(uniqueLikes); // ['romance'] - можно рекомендовать пользователю 1
```

### Пример 5: Управление доступом

```typescript
// Роли пользователей
await redis.sadd('role:admin:permissions', 'read', 'write', 'delete', 'manage');
await redis.sadd('role:user:permissions', 'read', 'write');
await redis.sadd('role:guest:permissions', 'read');

// Найти все разрешения (объединение)
const allPermissions = await redis.sunion(
  'role:admin:permissions',
  'role:user:permissions',
  'role:guest:permissions'
);
console.log(allPermissions); // Все уникальные разрешения

// Найти разрешения, доступные только админу (разность)
const adminOnly = await redis.sdiff(
  'role:admin:permissions',
  'role:user:permissions',
  'role:guest:permissions'
);
console.log(adminOnly); // ['delete', 'manage'] - только для админа

// Переместить пользователя из одной роли в другую
await redis.smove('role:user:users', 'role:admin:users', 'user:123');
```

## Итог

**Множество в Redis** - это неупорядоченная коллекция уникальных строк, идеально подходящая для:
- Тегов и категорий
- Уникальных идентификаторов
- Черных/белых списков
- Избранного
- Отслеживания уникальных событий
- Любых случаев, где важна уникальность и быстрый поиск

Множества обеспечивают автоматическую уникальность элементов и очень быструю проверку принадлежности элемента к множеству.

